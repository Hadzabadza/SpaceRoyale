void keyPressed() { //Here are all the hard-coded buttons for keyboard controls.
  if (!ships[0].displayPlanetMap) {
    ...
    if ((key == 'v')||(key=='V')) ships[0].landingMode(true);
  }
...
void keyReleased() {
  ...
  if (!ships[0].displayPlanetMap) {
    ...
    if ((key == 'v')||(key=='V')) ships[0].landingMode(false);
    if ((key == 'z')||(key=='Z')) {
      ships[0].fireMissile();
      ships[0].missileAiming=false;
    }
  } else if ((key == 'i')||(key=='I')) {
    ships[0].orbited.surfaceScreen.resourceMap=!ships[0].orbited.surfaceScreen.resourceMap;
  }
  if (ships.length>1&&!ships[1].displayPlanetMap) {
...
void mousePressed() {
  if (mouseButton==LEFT) if (testSLD.checkClicked()&&ships[0].displayPlanetMap) operated=testSLD;
}
...
void mouseReleased() {
  ...
  if (mouseButton==LEFT)
  {
    if (ships[0].displayPlanetMap) 
    {
      if (operated!=null) {
        operated.snap();
        ships[0].orbited.surfaceScreen.shownResource=byte(round(operated.power)-1);
        operated=null;
      } else {
