class OscHub { //<>// //<>// //<>// //<>// //<>// //<>//
  ...
  OscHub(int controllers) { //Hub that computes the messages before sending
    ...
    //initializeHub();
    OMPlanets=stars.get(0).planets.length;
    longestDistance=round(stars.get(0).planets[OMPlanets-1].distance);
...
class OscDock {
  ...
  OscDock(OscHub _hub, int _id, NetAddress _c, int port, Ship _s) {
    ...
    for (int i=0; i<hub.OMPlanets; i++) OMPlanetDistances[i]=round(stars.get(0).planets[i].distance/hub.longestDistance);
...
class OscDockInitialized extends OscDock {
  ...
  void OMPlanetMove(OscBundle outBundle, int targetPlanetIndex) {
    if (frameCount%Settings.planetLocationUpdateInterval==planetLocationUpdatePhase) {
      int OMPlanetPosX=round(stars.get(0).planets[targetPlanetIndex].pos.x/hub.longestDistance+OMCenterX+hub.OMLEDCorrectionOffset);
      outBundle.add(new OscMessage("/OM/planet"+targetPlanetIndex+"/position/x").add(OMPlanetPosX));
      int OMPlanetPosY=round(stars.get(0).planets[targetPlanetIndex].pos.y/hub.longestDistance+OMCenterY+hub.OMLEDCorrectionOffset);
